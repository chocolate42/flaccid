...
int gasc_main(void *input, size_t input_size, FILE *fout, flac_settings *set){
	...
	cstart=clock();
	if(!simple_enc_eof(&q, &a, set, input, &curr_sample, tot_samples, 2*set->blocksize_limit_lower, &stat, &ctx, fout)){//if not eof, init
	...
	while(curr_sample<tot_samples){
		if((a->outbuf_size+b->outbuf_size)<ab->outbuf_size){//dump a naturally
			...
			if(!simple_enc_eof(&q, &a, set, input, &curr_sample, tot_samples, 2*set->blocksize_limit_lower, &stat, NULL, fout)){//if next !eof, iterate
				swap=a;
				a=b;
				b=swap;
			...
			if(!simple_enc_eof(&q, &a, set, input, &curr_sample, tot_samples, 2*set->blocksize_limit_lower, &stat, &ctx, fout)){//if next !eof, iterate
